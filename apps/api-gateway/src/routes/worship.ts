...
export async function worshipRoutes(fastify: FastifyInstance) {
  fastify.get('/songs', async (r, reply) => {
  ...
  fastify.post('/songs', async (r, reply) => reply.status(201).send(await worshipClient.post('/worship/songs', r.body)));
  fastify.put('/songs/:id', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.put(`/worship/songs/${r.params.id}`, r.body)));
  fastify.post('/songs/:id/transpose', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.post(`/worship/songs/${r.params.id}/transpose`, r.body)));
  ...
  fastify.get('/playlists', async (r, reply) => {
    const q = r.query as Record<string, string>;
    const params = new URLSearchParams(q).toString();
  ...
  fastify.post('/playlists', async (r, reply) => reply.status(201).send(await worshipClient.post('/worship/playlists', r.body)));
  fastify.put('/playlists/:id', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.put(`/worship/playlists/${r.params.id}`, r.body)));
  ...
  fastify.get('/worship-events', async (r, reply) => {
    const q = r.query as Record<string, string>;
    const params = new URLSearchParams(q).toString();
    const path = `/worship/worship-events${params ? '?' + params : ''}`;
    reply.send(await worshipClient.get(path));
  });
  fastify.get('/worship-events/:id', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.get(`/worship/worship-events/${r.params.id}`)));
  ...
  fastify.post('/worship-events', async (r, reply) => reply.status(201).send(await worshipClient.post('/worship/worship-events', r.body)));
  fastify.put('/worship-events/:id', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.put(`/worship/worship-events/${r.params.id}`, r.body)));
  fastify.put('/worship-events/:id/songs', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.put(`/worship/worship-events/${r.params.id}/songs`, r.body)));
  fastify.put('/worship-events/:id/musicians', async (r: FastifyRequest<{ Params: { id: string } }>, reply) => reply.send(await worshipClient.put(`/worship/worship-events/${r.params.id}/musicians`, r.body)));
  fastify.post('/worship-events/:id/musicians/:memberId/confirm', async (r: FastifyRequest<{ Params: { id: string; memberId: string } }>, reply) => { await worshipClient.post(`/worship/worship-events/${r.params.id}/musicians/${r.params.memberId}/confirm`, r.body || {}); return reply.send({ success: true }); });
