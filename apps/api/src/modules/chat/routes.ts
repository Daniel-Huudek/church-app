...
export async function chatRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/:id', async (request, reply: FastifyReply) => {
    const { userId } = requireAuthUser(request);
    try {
      const room = await chatService.getRoom((request.params as any).id, userId);
      return reply.send({ success: true, data: room });
    } catch (error: any) {
      return reply.status(error.statusCode || 500).send({ success: false, message: error.message });
    }
  ...
  fastify.get('/:id/messages', async (request, reply: FastifyReply) => {
    const { userId } = requireAuthUser(request);
    const { page = '1', limit = '50' } = request.query as Record<string, string>;
    try {
      const result = await chatService.getMessages((request.params as any).id, userId, parseInt(page), parseInt(limit));
  ...
  fastify.post('/:id/messages', async (request, reply: FastifyReply) => {
    const { userId } = requireAuthUser(request);
    ...
    try {
      const message = await chatService.sendMessage((request.params as any).id, userId, content, type);
  ...
  fastify.post('/:id/read', async (request, reply: FastifyReply) => {
    const { userId } = requireAuthUser(request);
    try {
      await chatService.markAsRead((request.params as any).id, userId);
