...
export async function financeRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/transactions/:id', async (request, reply: FastifyReply) => {
    assertFinanceWriteRole(requireAuthUser(request).role);
    const data = await service.findTransactionById((request.params as any).id);
    return reply.send(data);
  });
  ...
  fastify.put('/transactions/:id', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    const body = validate(transactionSchema.partial(), request.body);
    const { userId, role } = requireAuthUser(request);
    const data = await service.updateTransaction((request.params as any).id, body, userId, role);
    return reply.send(data);
  });

  fastify.delete('/transactions/:id', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    const { userId, role } = requireAuthUser(request);
    await service.deleteTransaction((request.params as any).id, userId, role);
    return reply.send({ success: true });
  });

  fastify.post('/transactions/:id/confirm', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    const { userId, role } = requireAuthUser(request);
    const data = await service.confirmTransaction((request.params as any).id, userId, role);
    return reply.send(data);
  });

  fastify.post('/transactions/:id/cancel', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    const { userId, role } = requireAuthUser(request);
    const data = await service.cancelTransaction((request.params as any).id, userId, role);
    return reply.send(data);
  });

  fastify.post('/transactions/:id/attachments', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    ...
    const data = await service.addAttachment((request.params as any).id, { filename: file.filename, buffer, mimetype: file.mimetype }, userId);
    return reply.status(201).send(data);
  });

  fastify.get('/transactions/:id/audit', async (request, reply: FastifyReply) => {
    assertFinanceWriteRole(requireAuthUser(request).role);
    const { page, limit } = parsePagination(request.query);
    const data = await service.getAuditLogs({ page, limit, transactionId: (request.params as any).id });
    return reply.send(data);
  });
  ...
  fastify.put('/categories/:id', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    const body = validate(categorySchema.partial(), request.body);
    const data = await service.updateCategory((request.params as any).id, body);
    return reply.send(data);
  });
  ...
  fastify.put('/cost-centers/:id', { preHandler: [authorize(...financeRoles)] }, async (request, reply: FastifyReply) => {
    const body = validate(costCenterSchema.partial(), request.body);
    const data = await service.updateCostCenter((request.params as any).id, body);
    return reply.send(data);
  });
